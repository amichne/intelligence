...
- `hooks/` holds provider-neutral hook metadata, implementations, and provider
  adapter configs.
- `garden/manifests/source-roots.json` lists local roots that may contain source
  primitives.
- `garden/manifests/promotions.json` records primitives copied into this repository and
  the source paths they came from.
- `garden/manifests/primitive-audits.json` records durable quality, schema,
  provenance, runtime, and cleanup-readiness decisions before those decisions
  are used to turn off older sources.
- `garden/manifests/discovered-primitives.json` is generated inventory evidence.
- `garden/manifests/consolidation-report.json` is the generated review queue derived
  from inventory evidence.
- `garden/manifests/source-review-decisions.json` records explicit trim decisions for
  generated duplicate-name groups. The validator keeps it synchronized with the
  generated queue.
- `garden/manifests/digest-review-decisions.json` records explicit trim decisions for
...
- `garden/manifests/source-root-decisions.json` records explicit root-level decisions
  for scanned roots that do not need or receive generated duplicate-name or
  duplicate-content review entries.
- `garden/manifests/plugin-coverage.json` is generated coverage evidence for the
...
- `garden/manifests/primitive-decision-coverage.json` is generated per-entry review
...
- `garden/manifests/review-completeness.json` is generated review evidence for the
  canonical source graph. It records whether each canonical primitive has a
  durable audit decision.
- `garden/manifests/source-root-retirement.json` is generated retirement evidence for
...
- `garden/manifests/source-turnoff-readiness.json` is generated readiness evidence for
...
- `garden/manifests/runtime-activation-plan.json` is generated dry-run activation
...
- `garden/manifests/runtime-activation-preflight.json` is generated filesystem
...
- `garden/manifests/runtime-activation-approvals.json` is generated approval evidence
...
- `garden/docs/consolidation-queue.md` summarizes the promotion, duplicate, and broken
  symlink queues.
- `garden/docs/plugin-coverage.md` summarizes canonical primitive coverage through
  referential plugins and marketplace entries.
- `garden/docs/primitive-decision-coverage.md` summarizes per-entry decision coverage
...
- `garden/docs/review-completeness.md` summarizes remaining canonical primitive audit
  gaps.
- `garden/docs/source-root-retirement.md` summarizes root-by-root retirement,
  retention, and review posture for the full scan boundary.
- `garden/docs/source-turnoff-readiness.md` summarizes whether source replacement is
  blocked, review-ready, approved for execution, or complete.
- `garden/docs/toolbox-readiness.md` maps the original toolbox objective to current
  generated evidence and names the remaining review or approval boundary.
- `garden/docs/runtime-activation-plan.md` summarizes dry-run activation operations,
  including backup, symlink, and manual marketplace import previews.
- `garden/docs/runtime-activation-preflight.md` summarizes current filesystem
  readiness and review blockers for those activation operations.
- `garden/docs/runtime-activation-approvals.md` summarizes which activation packets
  are ready for approval, ready for manual import, or still need review.
- `garden/manifests/cleanup-ledger.json` is the approval gate before deletion,
...
1. Run `python3 garden/scripts/inventory-primitives.py` to refresh the local primitive
   inventory.
2. Run `python3 garden/scripts/analyze-consolidation.py` to rebuild the consolidation
   queue.
3. Review authored/local candidates before importing runtime or cache material.
4. Run `python3 garden/scripts/analyze-plugin-coverage.py` to rebuild canonical
   primitive coverage evidence.
5. Run `python3 garden/scripts/analyze-primitive-decision-coverage.py` to rebuild
   per-entry review coverage evidence for the discovered inventory.
6. Run `python3 garden/scripts/analyze-review-completeness.py` to rebuild canonical
   primitive audit-completeness evidence.
7. Run `python3 garden/scripts/analyze-source-turnoff-readiness.py` to rebuild the
   source turnoff readiness view.
8. Run `python3 garden/scripts/analyze-runtime-activation.py` to rebuild the dry-run
   activation plan.
9. Run `python3 garden/scripts/preflight-runtime-activation.py` to inspect the dry-run
   activation plan against the current filesystem.
10. Run `python3 garden/scripts/analyze-runtime-activation-approvals.py` to rebuild
   approval packets before asking for or acting on runtime activation approval.
11. Run `python3 garden/scripts/analyze-source-root-retirement.py` to rebuild
...
14. Record the source of promoted primitives in `garden/manifests/promotions.json`.
15. Record quality and readiness decisions in `garden/manifests/primitive-audits.json`
   when they will guide promotion, activation, or cleanup.
16. Record source-review decisions in
   `garden/manifests/source-review-decisions.json` before treating generated duplicate
   groups as trim candidates.
17. Record digest-review decisions in
   `garden/manifests/digest-review-decisions.json` before treating byte-identical
   groups as runtime replacement or cleanup candidates.
18. Record source-root decisions in `garden/manifests/source-root-decisions.json`
   when a scanned root needs manual retain/promote/review handling outside
   generated duplicate queues.
19. Record cleanup intent in `garden/manifests/cleanup-ledger.json` only after the
