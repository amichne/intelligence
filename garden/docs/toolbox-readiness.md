...
|---|---|---|---|
| `INCOMPLETE` | `canonical-primitive-set` | Canonical primitives: 51.<br>Audited canonical primitives: 50/51.<br>Needs audit: 1. | Keep adding primitive audits before treating any new canonical primitive as ready. |
| `INCOMPLETE` | `referential-plugins` | Referential plugins: 11.<br>All canonical primitives routed: false.<br>Coverage statuses: MARKETPLACE_EXPOSED=9, PLUGIN_COMPOSED=32, SCOPED_INSTRUCTION=7, STANDALONE_ONLY=3. | Route remaining standalone canonical primitives through plugins, marketplace entries, or scoped instructions. |
...
| `NEEDS_REVIEW` | `comprehensive-source-review` | Discovered primitive entries: 871.<br>Unreviewed singleton entries: 0.<br>Cleanup proposal gaps: 0.<br>Open source review groups: 0.<br>Open digest review groups: 0. | Resolve unreviewed entries, cleanup proposal gaps, or open review groups. |
...
| `NEEDS_APPROVAL` | `runtime-activation` | Already active packets: 15.<br>Ready for approval packets: 2.<br>Ready for manual import packets: 1.<br>Review-required packets: 0.<br>Blocked packets: 0. | Approve READY_FOR_APPROVAL packets by name only when the backup and symlink previews are acceptable; REVIEW_REQUIRED packets still need inspection. |
