...
GENERATORS = [
    ...
    ["python3", "garden/scripts/analyze-source-cleanup-gaps.py"],
...
CHECKS = [
    ...
    ["python3", "garden/scripts/analyze-source-cleanup-gaps.py", "--check"],
...
def check_python_syntax(repo_root: Path) -> int:
    ...
    files.extend(sorted(Path(path) for path in glob.glob("garden/scripts/*.py")))
    files.extend(
        [
