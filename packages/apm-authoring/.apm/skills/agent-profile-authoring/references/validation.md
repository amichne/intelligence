...
- Add the agent to `the root `apm.yml` `marketplace.packages` block` only after the file
  exists under `.apm/agents/`.
- Compose it into APM package manifests by reference.
- Record primitive provenance in public-safe form.
- Validate manifests.
