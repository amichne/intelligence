...
Use this skill to create independent agent profiles. An APM package may ship an
agent profile, but the agent itself must carry its role, trigger, process, tool
boundaries, and output contract.

## Operating Contract

- Make the agent profile useful without any package README or marketplace text.
...
- Keep primitive provenance public-safe when material comes from another source.
...
2. Check existing agents.
   Search `.apm/agents/` for overlapping names, triggers, or responsibilities.
...
6. Validate and promote.
   ...
   an APM package under `.apm/agents/`, then expose the package through the root
   `apm.yml` marketplace block when it should be discoverable.
...
- The agent remains independent of any APM package that ships it.
